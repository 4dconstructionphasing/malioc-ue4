...
public:
    ...
    struct FOSCCompilationProgress
    {
    ...
    /**
     * This should be called only when GetProgress() returns OSC_COMPILATION_IN_PROGRESS.
     * @return a structure containing the total shaders to be compiled, and the number of shaders we have already compiled
     */
    FOSCCompilationProgress GetOSCCompilationProgress() const;
     ...
     */
    TSharedRef<FMaliSCReport> GetReport() const;
...
private:
    ...
    mutable TSharedPtr<FMaliSCReport> CachedReport = nullptr;
