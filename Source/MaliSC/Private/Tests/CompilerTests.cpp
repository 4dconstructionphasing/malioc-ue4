...
#include "../MaliSCCompilerManager.h"
#include "../MaliSCAsyncCompiler.h"
#include "../MaliSCAsyncReportGenerator.h"
#include "AutomationTest.h"
#include "ShaderCompiler.h"

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMaliSCBlockUntilAllShaderCompilationCompleteTest, "MaliSC.BlockUntilAllShaderCompilationComplete", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FMaliSCBlockUntilAllShaderCompilationCompleteTest::RunTest(const FString& Parameters)
{
    GShaderCompilingManager->FinishAllCompilation();
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMaliSCCompilerManagerTest, "MaliSC.CompilerManager", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

// Test all the functionality of Compiler Manager wrapper which isn't already tested in the Async compiler tests
bool FMaliSCCompilerManagerTest::RunTest(const FString& Parameters)
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMaliSCCompilerLoadingTest, "MaliSC.CompilerLoading", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

// This indirectly tests a lot of the functionality exposed by the compiler manager
// It doesn't make sense to test it twice
bool FMaliSCCompilerLoadingTest::RunTest(const FString& Parameters)
{
...
struct FMaliSCAsyncReportGenerationParams
{
    int32 core;
    int32 rev;
    int32 dri;
        ...
        for (int j = 0; j < revs.Num(); j++)
        {
            ...
            for (int k = 0; k < drivers.Num(); k++)
            {
                ...
                for (int l = 0; l < platforms.Num(); l++)
                {
                    ...
                    FMaliSCAsyncReportGenerationParams params;
                    params.core = i;
                    params.rev = j;
                    params.dri = k;
                    params.plat = l;
...
IMPLEMENT_COMPLEX_AUTOMATION_TEST(FMaliSCCompilationReportTest, "MaliSC.CompilationReport", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

void FMaliSCCompilationReportTest::GetTests(TArray<FString>& OutBeautifiedNames, TArray <FString>& OutPlatformStrings) const
{
...
struct FMaliSCCompilationReportTestData
{
    FMaliSCCompilationReportTest* test;
    TSharedRef<FAsyncReportGenerator> reportGenerator;
};

DEFINE_LATENT_AUTOMATION_COMMAND_ONE_PARAMETER(FMaliSCCompilationReportTest_WaitForCompilationToComplete, FMaliSCCompilationReportTestData, testData);

bool FMaliSCCompilationReportTest::RunTest(const FString& Parameters)
{
    if (FCompilerManager::Get() == nullptr)
    {
        return false;
    ...
    FMaliSCCompilationReportTestData testData = { this, reportGenerator };

    // Checks each frame to see if compilation is complete, and when it is ensures output is as expected
    ADD_LATENT_AUTOMATION_COMMAND(FMaliSCCompilationReportTest_WaitForCompilationToComplete(testData));

    return true;
}

bool FMaliSCCompilationReportTest_WaitForCompilationToComplete::Update()
{
    ...
    auto progress = testData.reportGenerator->GetOSCCompilationProgress();
