...
void FAsyncReportGenerator::Tick(float DeltaTime)
{
    ...
    if (Progress == EProgress::CROSS_COMPILATION_IN_PROGRESS)
    {
        ...
        // Start the async OSC job
        check(!JobHandle.IsValid());
        JobHandle = FAsyncCompiler::Get()->AddJob(Resource->GetGameThreadShaderMap(), Platform);
        Progress = EProgress::OSC_COMPILATION_IN_PROGRESS;
    }

    if (Progress == EProgress::OSC_COMPILATION_IN_PROGRESS)
    {
        // Wait until the OSC job is complete
...
void FAsyncReportGenerator::FinishReportGeneration()
{
    ...
    if (Progress == EProgress::CROSS_COMPILATION_IN_PROGRESS)
    {
        ...
        Tick(0.0f);
        check(Progress == EProgress::OSC_COMPILATION_IN_PROGRESS);
    }
...
FAsyncReportGenerator::FOSCCompilationProgress FAsyncReportGenerator::GetOSCCompilationProgress() const
{
    FOSCCompilationProgress ret;
...
FMidgardBoundPipes GetMidgardBoundPipes(const FMaliSCRawCompilerOutput::FMidgardOutput::FRenderTarget& RenderTarget)
{
...
TArray<TSharedRef<FString>> GetDetailsFromCommonOutput(const FMaliSCRawCompilerOutput::FCommonOutput& CommonOutput)
{
...
TSharedRef<FMaliSCReport> FAsyncReportGenerator::GetReport() const
{
    check(Progress == EProgress::COMPILATION_COMPLETE);
    ...
    CachedReport = MakeShareable(new FMaliSCReport);
    ...
    if (bWasCompilationError)
    {
        ...
        TSharedRef<FMaliSCReport::FErrorReport> errorReport = MakeShareable(new FMaliSCReport::FErrorReport);
    ...
    else
    {
        ...
        {
            TSharedRef<FMaliSCReport::FMidgardReport> report = MakeShareable(new FMaliSCReport::FMidgardReport);
            ...
            {
                TSharedRef<FMaliSCReport::FMidgardReport::FRenderTarget> rtReport = MakeShareable(new FMaliSCReport::FMidgardReport::FRenderTarget);
            ...
            {
                TSharedRef<FMaliSCReport::FMidgardReport> reportCopy = MakeShareable(new FMaliSCReport::FMidgardReport(*report));
        ...
        {
            TSharedRef<FMaliSCReport::FUtgardReport> report = MakeShareable(new FMaliSCReport::FUtgardReport);
            report->TitleName = output.CommonOutput.ShaderName;
            report->VertexFactoryName = output.CommonOutput.VertexFactoryName;
            report->Details = GetDetailsFromCommonOutput(output.CommonOutput);
            report->SourceCode = output.CommonOutput.SourceCode;
            ...
            if (shaderDescription != nullptr)
            {
                TSharedRef<FMaliSCReport::FUtgardReport> reportCopy = MakeShareable(new FMaliSCReport::FUtgardReport(*report));
        ...
        const auto errorSorter = [](const TSharedRef<FMaliSCReport::FErrorReport>& a, const TSharedRef<FMaliSCReport::FErrorReport>& b) -> bool
        {
            return a->TitleName < b->TitleName;
        };
        CachedReport->ErrorList.Sort(errorSorter);

        const auto midgardSorter = [](const TSharedRef<FMaliSCReport::FMidgardReport>& a, const TSharedRef<FMaliSCReport::FMidgardReport>& b) -> bool
        {
            return a->TitleName < b->TitleName;
        };
        CachedReport->MidgardReports.Sort(midgardSorter);

        const auto utgardSorter = [](const TSharedRef<FMaliSCReport::FUtgardReport>& a, const TSharedRef<FMaliSCReport::FUtgardReport>& b) -> bool
        {
            return a->TitleName < b->TitleName;
        };
