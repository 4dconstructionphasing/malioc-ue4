...
static const FString MALISC_FOLDER_PATH = FPaths::ConvertRelativePathToFull(FPaths::Combine(*FPaths::EnginePluginsDir(), TEXT("Editor/"), TEXT("MaliSC/")));

static const FString FULL_COMPILER_PATH = FPaths::Combine(*MALISC_FOLDER_PATH, *OFFLINE_COMPILER_FOLDER_TO_EXTRACT);
...
const FString& FCompilerManager::GetMaliSCFolderPath()
{
    return MALISC_FOLDER_PATH;
}
...
bool FCompilerManager::Initialize(bool Silent)
{
    ...
    if (!CompilerManager->bIsValid)
    {
        if (!Silent)
        {
            UE_LOG(MaliShaderCompiler, Error, TEXT("Failed to load compiler manager DLL"));
        }
    ...
    if (!success)
    {
        if (!Silent)
        {
            UE_LOG(MaliShaderCompiler, Error, TEXT("Could not initialize compiler libraries"));
        }
        CompilerManager.Reset();
        return false;
    }
    ...
    if (version.major != expectedVersion.major || version.minor != expectedVersion.minor || version.patch != expectedVersion.patch)
    {
        if (!Silent)
        {
            UE_LOG(MaliShaderCompiler, Error, TEXT("Compiler manager version is not expected version"));
        }
        CompilerManager.Reset();
        return false;
    }
