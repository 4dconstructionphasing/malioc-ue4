...
bool FAsyncCompiler::Initialize(bool Silent)
{
    ...
    if (!success)
    {
        if (!Silent)
        {
            UE_LOG(MaliShaderCompiler, Error, TEXT("Could not initialise Async Compiler as compiler libraries were not successfully loaded"));
        }
        return false;
    }
...
void FCompileJobHandle::BeginCompilationAsync()
{
    ...
    Thread = FRunnableThread::Create(this, *FString::Printf(TEXT("MaliSCCompileJob %d"), JobCounter));
    JobCounter += 1;
}
...
uint32 FCompileJobHandle::Run()
{
    for (const TPair<FShaderId, FShader*> shader : OutShaders)
    {
        ...
        {
            FMaliSCRawCompilerOutput::FErrorOutput error;
...
FMaliSCRawCompilerOutput::FMidgardOutput ParseMidgardFlexibleOutputs(const malioc_key_value_pairs* const flexible_outputs, unsigned int size)
{
    FMaliSCRawCompilerOutput::FMidgardOutput output;

    for (unsigned int i = 0; i < size; ++i)
    {
        FMaliSCRawCompilerOutput::FMidgardOutput::FRenderTarget rt;
...
FMaliSCRawCompilerOutput::FUtgardOutput ParseUtgardFlexibleOutputs(const malioc_key_value_pairs* const flexible_outputs, unsigned int size)
{
    FMaliSCRawCompilerOutput::FUtgardOutput output;
...
{
    FMaliSCRawCompilerOutput::FCommonOutput commonOutput;
    ...
    {
        FMaliSCRawCompilerOutput::FMidgardOutput MidgardOutput = ParseMidgardFlexibleOutputs(outputs.flexible_outputs, outputs.number_of_flexible_outputs);
    ...
    {
        FMaliSCRawCompilerOutput::FUtgardOutput UtgardOutput = ParseUtgardFlexibleOutputs(outputs.flexible_outputs, outputs.number_of_flexible_outputs);
